...
export default function ThreejsMesh({ pol, selectedData, showPower }) {
  ...
  function addAxisLinesAndLabels(scene, inputLength) {
    const axisLength = inputLength;
  ...
  useEffect(() => {
    ...
    geometry.setAttribute(
      "position",
      new THREE.Float32BufferAttribute(cartesianData.flat(), 3)
    );
    geometry.setAttribute(
      "color",
    ...
    // Removes previous mesh from the scene and disposes of its resources
    if (surfaceRef.current) {
    ...
    // Creates the new mesh with vertex colors and adds it to the scene
